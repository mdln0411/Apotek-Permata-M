            ...
            <tbody class="text-sm divide-y divide-slate-50">
                ...
                <tr class="hover:bg-slate-50 transition-colors">
                    ...
                    <td class="px-8 py-5 text-right">
                        <form action="/admin/users/{{ $user->id }}" method="POST" onsubmit="return confirm('Apakah anda yakin ingin menghapus user ini? Semua data terkait (pesanan, chat) akan ikut terhapus.')">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all {{ $user->id == auth()->id() ? 'opacity-0 pointer-events-none' : '' }}">
